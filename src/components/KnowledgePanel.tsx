        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={rescan}
              disabled={busy}
              className="rounded border border-border px-2 py-1 text-[11px] text-zinc-400 hover:border-zinc-600 hover:text-zinc-200 disabled:opacity-40"
            >
              {busy ? "…" : "↻ Rescan"}
            </button>
            <button
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mb-3">
            ...
            {rulesEmpty ? (
              ...
              <pre className="max-h-40 overflow-y-auto rounded border border-border bg-panel px-3 py-2 text-[10px] leading-relaxed whitespace-pre-wrap text-zinc-400">
                {report!.rules}
              </pre>
            )}
          </div>
            ...
            {!report || report.skills.length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5">
                {report.skills.map((s) => (
                  <label
                    key={s.name}
                    className="flex cursor-pointer items-start gap-2.5 rounded border border-border bg-panel px-3 py-2 hover:border-zinc-600"
                  >
                    <input
                      type="checkbox"
                      checked={s.active}
